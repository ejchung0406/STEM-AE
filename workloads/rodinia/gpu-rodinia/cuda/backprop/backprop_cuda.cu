...
void bpnn_train_cuda(BPNN *net, float *eo, float *eh)
{
  ...
  float sum;
  float *input_weights_one_dim;  float *input_weights_prev_one_dim;
  int original_num_blocks = in / 16;  
  
  // Handle large grid sizes by using 2D grid layout
  int max_grid_dim = 65535; // Maximum grid dimension for most CUDA devices
  int grid_x = 1;
  int grid_y = original_num_blocks;
  
  if (original_num_blocks > max_grid_dim) {
    grid_x = (original_num_blocks + max_grid_dim - 1) / max_grid_dim;
    grid_y = max_grid_dim;
  }
  
  num_blocks = original_num_blocks; // Keep original for memory allocation
  ...
  cudaMemcpy(input_cuda, net->input_units, (in + 1) * sizeof(float), cudaMemcpyHostToDevice);  cudaMemcpy(input_hidden_cuda, input_weights_one_dim, (in + 1) * (hid + 1) * sizeof(float), cudaMemcpyHostToDevice);

  
  printf("grid, threads = (%d, %d), (%d, %d), total_blocks = %d\n", grid.x, grid.y, threads.x, threads.y, num_blocks);
